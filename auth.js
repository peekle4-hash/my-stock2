...
function getSbConfig() {
  return {
    ...
    key: localStorage.getItem(CFG_KEY_KEY) || '',
  };
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (!ready) {
    // 설정 미완료: 안내만 표시
    setLoginMsg('먼저 Supabase 설정을 해주세요.', true);
  } else {
    // 세션 확인
  ...
  document.getElementById('loginGoogleBtn').addEventListener('click', async () => {
    if (!_supabase) { showSetupModal(); return; }
